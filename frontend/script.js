...
async function fetchCollections() {
    try {
        ...
        collectionSelect.innerHTML = collections.map(c => `<option value="${c}">${c}</option>`).join("");
    } catch (err) {
        console.error(err);
...
async function loadCollectionData() {
    ...
    if (!selectedCollection) {
        docsList.innerHTML = "<i>Nenhuma coleção selecionada.</i>";
        return;
    }

    docsList.innerHTML = "Carregando documentos...";
    formContainer.innerHTML = "";
    resultArea.textContent = "";
    editingDocId = null;
    btnUpdate.disabled = true;
    btnReplace.disabled = true;
    formTitle.textContent = "Inserir / Editar";

    try {
        const docs = await fetchJson(`${API}/documents/${encodeURIComponent(selectedCollection)}`);
        currentDocs = docs;
        renderDocs(docs);

        const template = await fetchJson(`${API}/template/${encodeURIComponent(selectedCollection)}`);
        currentTemplate = template;
        buildFormFromTemplate(template);

    } catch (err) {
        console.error("Erro ao carregar dados da coleção:", err);
        docsList.innerHTML = `<span style="color: red;">Falha ao carregar: ${err.message}</span>`;
        alert("Erro ao carregar dados da coleção: " + err.message);
    }
}
...
function renderDocs(docs) {
    ...
    docs.forEach(doc => {
        ...
        btnDelete.onclick = async () => {
            ...
            try {
                const result = await fetchJson(`${API}/delete/${encodeURIComponent(collectionSelect.value)}/${encodeURIComponent(doc._id)}`, {
                    method: "DELETE"
                });
    ...
    function _buildFields(obj, parentEl, namePrefix) {
        for (const key of Object.keys(obj)) {
            ...
            if (typeof sample === "boolean") {
                setByPath(initialData, fieldName, true);
                continue;
            }
            ...
            if (typeof sample === "object" && sample !== null && !Array.isArray(sample)) {
              ...
              parentEl.appendChild(fieldGroup);
            }
        }
    }
    _buildFields(template, root, "");
    formContainer.appendChild(root);
}
